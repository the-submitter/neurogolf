"""Generate the task-number/ID/principle map from ARC-GEN task_list comments."""
...
def build_mapping(repository_root: Path) -> dict[str, dict[str, str]]:
    canonical: Any = json.loads(
        (repository_root / "configs/task_map.json").read_text(encoding="utf-8")
    )
    if not isinstance(canonical, dict):
        raise ValueError("configs/task_map.json must contain an object")
    ...
    for task_key, task_id in canonical.items():
        if not isinstance(task_key, str) or not isinstance(task_id, str):
            raise ValueError("configs/task_map.json contains a non-string mapping")
...
def main() -> int:
    ...
    destination = root / "configs/task_principles.json"
